/*
This header file contains the core benchmark infrastructure.

Purpose:
--------
It defines all fundamental data structures and classes required to
execute queue benchmarks, including:

- Config: describes a benchmark configuration (threads, batches, timing)
- Results: aggregated benchmark results
- Counter: per-thread performance counters
- Benchmark: the main class that executes benchmarks on a given queue

This file is independent of a specific queue implementation and operates
on the abstract BaseQueue interface.

Why this file is necessary:
---------------------------
This file provides a reusable and extensible benchmark framework that can
be applied to multiple queue implementations without code duplication.

It cleanly separates:
- benchmark logic
- configuration
- result aggregation
from the actual queue implementations.
*/
...
void update_results(Results &res, std::vector<Counter> const &counters)
{
    res.total_n_operations = std::accumulate(
        counters.begin(), counters.end(), size_t(0),
    ...
    res.total_enqueues = std::accumulate(counters.begin(), counters.end(), size_t(0),
                                         [](size_t sum, Counter const &c)
                                         { return sum + c.total_push; });

    res.total_dequeues = std::accumulate(counters.begin(), counters.end(), size_t(0),
                                         [](size_t sum, Counter const &c)
...
public:
    const Results& get_results_ref() const { 
    return results; 
    }
...
#pragma omp parallel num_threads(config.num_threads)
            {
                ...
                if (enqueue_batch_size > 0) {
                    ...
                    push_elements = generate_batch_of_elements(enqueue_batch_size, thread_rng);
                ...
                l_counter.total_operations = l_counter.total_pop + l_counter.total_push;
...
#pragma omp barrier
                ...
                for (size_t i{0}; i < config.sets; ++i)
                {
                    ...
                    if (dequeue_batch_size > 0) {
                        ...
                        {
                            value_t val = queue.pop();
                            if (val != generics::empty_val)
                                l_counter.succeeded_pop++;
                        }
                        l_counter.total_pop += dequeue_batch_size;
                    }
...
#pragma omp barrier
    ...
    lock_free_aba::CASCounter run_fast_cache_checks(lock_free_aba::Queue &queue)
    {
        std::mt19937 global_rng(config.seed);
        std::vector<lock_free_aba::CASCounter> cas_counters;

        counters.resize(config.num_threads);
        cas_counters.resize(config.num_threads);
...
#pragma omp parallel num_threads(config.num_threads)
            {
                uint thread_id = omp_get_thread_num();
                Counter &l_counter = counters[thread_id];
                lock_free_aba::CASCounter &l_cas_counter = cas_counters[thread_id];
                reset_counter(l_counter);
...
#pragma omp barrier
                ...
                for (size_t i{0}; i < SETS; ++i)
                {
                    ...
                    for (size_t i = 0; i < enqueue_batch_size; i++)
                    {
                        // if (queue.push(push_elements[i]))
                        //     l_counter.succeeded_push++;
                        queue.pushb(push_elements[i], l_cas_counter);
                    }
                    l_counter.total_push += enqueue_batch_size;
                    l_counter.succeeded_push += enqueue_batch_size;
...
#pragma omp barrier
        ...
        lock_free_aba::CASCounter total{};

        total.success = std::accumulate(cas_counters.begin(), cas_counters.end(), 0, [](size_t sum, lock_free_aba::CASCounter const &a)
                                        { return sum + a.success; });
        total.failures = std::accumulate(cas_counters.begin(), cas_counters.end(), 0, [](size_t sum, lock_free_aba::CASCounter const &a)
                                         { return sum + a.failures; });
